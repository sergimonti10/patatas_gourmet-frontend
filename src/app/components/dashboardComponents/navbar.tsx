...
import {
    ...
    NavbarContent, NavbarItem, Image, Divider, Tooltip, Dropdown,
    DropdownTrigger, Avatar, DropdownItem, DropdownMenu, Badge
...
export default function NavBar() {
    ...
    // carrito como MAPA id->qty y contador desde el store
    const cartMap = useCartStore((s) => s.cart);
    const initializeCart = useCartStore((s) => s.initializeCart);
    const cartCount = useCartStore((s) => s.getCartCount()) as number;
    ...
    console.log("Contenido del carrito (mapa): ", cartMap);
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                ...
                const data = await response.json();
                if (data.image) setImgUser(`${data.image}`);
            } catch (error) {
                console.error('Error fetching user:', error);
            }
        };
    ...
    return (
        <Navbar isBordered isMenuOpen={isMenuOpen} onMenuOpenChange={setIsMenuOpen} className='bg-amber-950 rounded-b-full'>
            ...
            <NavbarContent className="sm:hidden mr-3" justify="center">
                <NavbarBrand>
                    <Link href='/' className='cursor-pointer'>
                        <Image isBlurred width={70} className="m-5" src="/images/logo.png" alt="Patatas Gourmet" />
                    </Link>
                </NavbarBrand>
            </NavbarContent>
            ...
            <NavbarContent className="hidden sm:flex lg:flex-1 gap-4 mx-auto" justify="start">
                <NavbarBrand className='lg:mr-10'>
                    <Link href='/' className='cursor-pointer'>
                        <Image isBlurred width={40} className="m-5" src="/images/logo.png" alt="Patatas Gourmet" />
                    </Link>
                </NavbarBrand>
            </NavbarContent>
                    ...
                    <React.Fragment key={link.name}>
                        ...
                        <NavbarItem className='text-amber-100'>
                            <Tooltip content={link.name} placement="bottom" className="block md:hidden">
                                <Link
                                    href={link.href}
                                    className={`flex flex-col items-center transition-all active:scale-95 hover:shadow-glow-amber-hover mx-2 p-1 rounded-lg ${pathname === link.href ? 'shadow-glow-amber' : ''}`}
                                >
                                    {link.name === 'Carrito' ? (
                                        <Badge
                                            content={<span>{cartCount}</span>} // 👈 evita error de tipos
                                            color="warning"
                                            variant="solid"
                                            isInvisible={cartCount === 0}
                                            placement="top-right"
            ...
            <NavbarMenu className='text-amber-950'>
                {links.map((link, index) => (
                    <NavbarMenuItem key={`${link.name}-${index}`}>
                        <Link
                            className={`flex items-center w-full text-xl ${fontClasses['font-unna']}`}
                            href={link.href}
